...
from deeplab import input_label_preprocess
...
def _get_data(data_provider, dataset_split):
  ...
  label, class_label, cdr_label = data_provider.get([common.LABELS_CLASS, common.CLASS_LABEL, common.CDR_LABEL]) #LABELS_CLASS])
  return image,  label, class_label, cdr_label, image_name, height, width
...
def get(dataset,
  ...
  data_provider = dataset_data_provider.DatasetDataProvider(
      ...
      shuffle=is_training)
  image,  label, class_label, cdr_label, image_name, height, width = _get_data(data_provider,
                                                      dataset_split)
  ...
  if label is not None:
    ...
    cdr_label = tf.expand_dims(cdr_label, 0)
  
  original_image, image, label, class_label, cdr_label = input_label_preprocess.preprocess_image_and_label(
      image,
      label, class_label, cdr_label,
  ...
  if label is not None:
    ...
    sample[common.CDR_LABEL] = cdr_label
