...
from deeplab import input_stack_preprocess
...
def _get_data(data_provider, dataset_split):
  ...
  height, width = data_provider.get(
      [common.HEIGHT, common.WIDTH])

  images = []
  segs = []
  for i in range(256):
    print(i, 256, chr(27) + '[A')
    image = data_provider.get(
      [common.IMAGE + '_{:04d}'.format(i)])
    images.append(image)
    if dataset_split != common.TEST_SET:
        seg = data_provider.get(
      [common.LABELS_CLASS + '_{:04d}'.format(i)])
        segs.append(seg)
  ...
  if dataset_split != common.TEST_SET:
    segs = tf.stack(segs, axis=1)
    class_label,  = data_provider.get([common.CLASS_LABEL]) #LABELS_CLASS])
  else:
    segs = None
    class_label = None
  return images, segs, image_name, height, width, class_label
...
def get(dataset,
  ...
  image, label, image_name, height, width, class_label = _get_data(data_provider, dataset_split)
  if label is not None:
    ...
    #  pass
    #else:
    #  raise ValueError('Input label shape must be [depth, height, width], or '
    #                   '[height, width, 1].')
    label.set_shape([None, None, None, None, 1])
    class_label.set_shape([None, None, 1])
  original_image, image, label, class_label = input_stack_preprocess.preprocess_image_and_label(
      image,
      label, class_label,
  ...
  if label is not None:
    ...
    sample[common.CLASS_LABEL] = class_label
